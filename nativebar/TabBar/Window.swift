...
class Window: NSWindow {
  ...
  var terminalController: TabBarController? {
    windowController as? TabBarController
  }
  ...
  override func awakeFromNib() {
    ...
    // Hide all traffic light buttons by default. The close button will be
    // shown by tabBarDidDisappear when there's only a single tab.
    hideWindowButtons()

    // AppKit may add the tab bar after addTabbedWindow returns, so
    // tabBarDidAppear/tabBarDidDisappear may not have fired yet.
    // Re-sync the close button state on the next tick.
    DispatchQueue.main.async { [weak self] in
      guard let self else { return }
      if self.tabBarView != nil {
        self.tabBarDidAppear()
      } else {
        self.tabBarDidDisappear()
      }
    }
    ...
    if styleMask.contains(.titled) {
      ...
      resetZoomAccessory.view = NSHostingView(
        rootView: ResetZoomAccessoryView(
          viewModel: viewModel,
          action: { [weak self] in
            guard let self else { return }
      ...
      //   updateAccessory.view = NonDraggableHostingView(
      //     rootView: UpdateAccessoryView(
      //       viewModel: viewModel,
  ...
  override func close() {
    print("[Window] | fn close()")
    print("window delegate:", self.delegate)
    print("window controller:", self.windowController)
    print("window controller type:", type(of: self.windowController))
  ...
  private func tabBarDidAppear() {
    // Tab bar has its own close buttons per tab — hide the traffic light close.
  ...
  private func tabBarDidDisappear() {
    // Single tab — show the traffic light close button so the user can close.
  ...
  private func generateResetZoomButton() -> NSButton {
    ...
    // button.action = #selector(TabBarController.splitZoom(_:))
  ...
  func syncAppearance() {
    ...
    // if !styleMask.contains(.fullScreen) && !forceOpaque
    ...
    // let backgroundColor = preferredBackgroundColor
    // ?? NSColor(surfaceConfig.backgroundColor)
...
extension Window {
  ...
  func configureTabContextMenuIfNeeded(_ menu: NSMenu) {
    ...
    //   .flatMap { $0.windowController as? TabBarController }

    // // Close tabs to the right
    // let item = NSMenuItem(
    //   title: "Close Tabs to the Right",
    //   action: #selector(TabBarController.closeTabsOnTheRight(_:)), keyEquivalent: "")
  ...
  private func appendTabModifierSection(to menu: NSMenu, target: TabBarController?) {
    ...
    //   title: "Change Title...", action: #selector(TabBarController.changeTabTitle(_:)),
    //   keyEquivalent: "")
    ...
    //   selectedColor: (target?.window as? Window)?.tabColor ?? .none
    // ) { [weak target] color in
    //   (target?.window as? Window)?.tabColor = color
    // }
...
private func makeTabColorPaletteView(
  ...
  selectionHandler: @escaping (TabColor) -> Void
) -> NSView {
  let hostingView = NSHostingView(
    rootView: TabColorMenuView(
